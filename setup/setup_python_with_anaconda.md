...
To set up Anaconda for our course, you need to take four steps:

1. install Anaconda
2. verify your Anaconda installation (python and conda should be ready to go)
3. create a new conda environment
4. install required packages in your conda environment
...
For this, you need to download the installer for your operating system (Windows or macOS), and then run the installer to configure Anaconda for use on your computer.
...
2. In step six described in the first link, please opt for the "Alternative Approach" (i.e., automatically add Anaconda to your PATH variable at installation time).

#### MacOS/Linux

Download the installer from https://www.anaconda.com/download/success#mac or follow the instructions in [setup_macos.md](./setup_macos.md).

*Notes:* 

- If your computer has Apple silicon (i.e., an M1/M2/... chip), choose "64-bit (Apple silicon) Graphical Installer". Otherwise, choose "64-bit (Intel chip) Graphical Installer".
...
**_Important note:_** When asked to select the destination of the installation (step 4 in the second link), please choose "Install for me only" (e.g., use your Applications folder).

### 2. Verify your Anaconda installation

Run the following commands in the Anaconda Prompt (Windows) or Terminal (macOS):
...
Both commands should run without errors and print version information.

If you want to verify the installer download itself, you can also follow the instructions here:
...
If you prefer the Anaconda Navigator app, you can create the environment there.
The Anaconda Navigator app should open when you finish your Anaconda installation.
...
- in step 5, use a Python version that starts with '3.11' (or higher)
...
The Navigator instructions above and the command-line instructions below are alternatives, so you only need to follow one of them.
...
So first, run the following command in the Anaconda Prompt (Windows) or Terminal (macOS):
...
If not, install the packages by running the following command in the Anaconda Prompt (Windows) or Terminal (macOS):
...
If you already have Python installed and prefer a lighter setup, you can also use [setup_dependencies_with_venv.md](./setup_dependencies_with_venv.md). That path is intended as a fallback for experienced users, and support may be limited if issues arise.
